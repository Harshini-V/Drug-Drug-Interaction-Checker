...
class Drug:
    ...
    def __init__(self, features):
        ...
        self.description = features['desc']
...
for i in tqdm(range(len(drugs))):
    ...
    for idx,feature in enumerate(drug):
        ...
        if 'drug-interactions' in str(feature): #interaction other drugs
            ...
            #     drug_interaction = ';'.join([di[0].text]) #';'.join([di[0].text
            #                        #         for di in list(drug[idx])])
            #     description = ';'.join([di[2].text])
            interaction_list = []
            description_list = []
            for di in list(drug[idx]):
                interaction_list.append(di[0].text)  # Interaction drug
                description_list.append(di[2].text)  # Interaction description
            drug_interaction = ';'.join(interaction_list)
            description = ';'.join(description_list)  # Join descriptions into a single string
